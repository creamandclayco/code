  ...
  function addToCart(){
    ...
    */
    ...
    if (document.querySelector("#itemPricingForm")){
        ...
        let itemEnvelopeColor = document.querySelector("input[name='Envelope-Color']:checked");
        if (itemEnvelopeAddressing){ itemEnvelopeAddressing = itemEnvelopeAddressing.value }
        if (itemEnvelopeColor){ 
            itemEnvelopeColor = itemEnvelopeColor.value 
        } else {
            itemEnvelopeColor = "";
        }

        itemData.type = "Paper";
        itemData.title = document.querySelector("[item-title]").getAttribute('item-title');
        itemData.slug = document.querySelector("[item-slug]").getAttribute('item-slug');
        itemData.collection = document.querySelector("[form_item='collection']").value;
        itemData.quantity = itemQuantity;
        itemData.quantityType = itemQuantityType;
        itemData.paper = document.querySelector("[form_item='paper']").value;
        itemData.basePrice = document.querySelector("[form_item_base_price]").getAttribute("form_item_base_price");
        itemData.price = document.querySelector("[form_item_final_price]").getAttribute("form_item_final_price");
        itemData.url = window.location.href; 
        itemData.envelopeAddressing = itemEnvelopeAddressing;
        itemData.envelopeColor = itemEnvelopeColor;
        itemData.id = `Paper-${itemData.title}-${itemData.collection}-${itemData.paper}-${itemData.envelopeColor}-${itemData.envelopeAddressing}`
    ...
    if (document.querySelector("#waxPricingForm")){
        ...
        itemData.type = "Wax";
        itemData.quantity = document.querySelector("[form_item='quantity']").value;
        itemData.quantityType = "Large";
        itemData.basePrice = document.querySelector("[form_item_base_price]").getAttribute("form_item_base_price");
        itemData.price = document.querySelector("[form_item_final_price]").getAttribute("form_item_final_price");
        itemData.waxColor = document.querySelector('input[name="Wax-Color"]:checked').value;
        itemData.waxDesign = document.querySelector('input[name="Wax-Design"]:checked').value;
        itemData.url = window.location.href; 
        itemData.title = itemData.waxDesign + " wax seals";
        itemData.slug = 'wax-seals';
        itemData.id = `Wax-${itemData.waxColor}-${itemData.waxDesign}`
        addToLocalStorage(itemData)
    }
